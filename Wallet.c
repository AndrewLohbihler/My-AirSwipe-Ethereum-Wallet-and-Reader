...
// MyWallet code
//
// This is the starter code for the Wallet device using a PIC16F1823 microprocessor
//
...
#pragma config LVP = ON	    // disable low voltage programming
...
unsigned int Nsample,Msample,Rsample,Lsample,Payment;
unsigned int State, Nevent, Mevent, Hevent, Revent, Levent;
...
void switch_action_PAY(void);
void switch_action_COLLECT(void);
...
//                transmit_code();		// transmit code
            ...
            case 1:
//                transmit_code();		// transmit code
                switch_action_PAY();		// perform switch action (IR only MASTER MODE)
                break;

            case 2:
//                transmit_code();		// transmit code
                switch_action_COLLECT();	// perform switch action (for CAP/TOUCH SLAVE MODE)
                break;
        }
...
//
    ...
    PORTA = 0b00000000;
    PORTC = 0b00010100;
    TRISA = 0b00111101;  //0 = Output, 1 = Input (RX is an input)
    TRISC = 0b00000010;  //0 = Output, 1 = Input (RX is an input)
...
//    CCP1CON = 0b00001100; // duty lowest bits + PWM mode
    ...
    STR1C = 0;   // Start the PWM steering as off for RIGHT_RF
    STR1D = 0;   // Start the PWM steering as off for LEFT_RF 
	FVRCON = 0b00000000;
}
...
// detect the trigger
//
    if (PAY == 1)         // was 1100 was 2400 (recently use 1800) 
	{
            Nsample++;
            Nevent = 0;
	}
...
//            Nsample = 0;
    ...
    // detect the trigger
//
    if (COLLECT == 1)         // was 1100 was 2400 (recently use 1800) 
	{
            Msample++;
            Mevent = 0;
	}
...
//
	...
	else
	{
        ...
        if (Nsample >= 5 && Nevent == 1)
        {
            ...
            NOP();
            Left_RF_Module();
            while(TXIF == 0); TXREG = 50;
            Nsample = 0;
            Nevent = 0;
        }
    
        if (Msample >= 5 && Mevent == 1)
        {
            SWITCH = 1;
            __delay_ms(100);
            SWITCH = 0;
...
void switch_action_PAY()
{
// detect the switch COLLECT trigger
//
    if (PAY == 1)         // was 1100 was 2400 (recently use 1800) 
	{
            Nsample++;
